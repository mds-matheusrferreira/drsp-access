...
class ManifestacaoController extends Controller
{
    ...
    public function edit(Request $request): View|RedirectResponse
    {
        ...
        return view('base-externa.analise-processo.manifestacao.edit', $this->viewData($protocolo));
    }
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        $payload = $request->only($this->accessProcesses->manifestacaoColumns());
        ...
        if (($validated['_action'] ?? 'save') === 'save_pdf') {
            return redirect()
                ->route('base-externa.analise-processo.manifestacao.pdf', ['protocolo' => $originalProtocolo])
                ->with('success', 'Manifestação atualizada com sucesso.');
        }

        return redirect()
            ->route('base-externa.analise-processo.manifestacao.edit', ['protocolo' => $originalProtocolo])
            ->with('success', 'Manifestação atualizada com sucesso.');
    }
    ...
    public function pdf(Request $request): Response|RedirectResponse
    {
        ...
        $pdf = Pdf::loadView('base-externa.analise-processo.manifestacao.pdf', $this->viewData($protocolo))
            ->setPaper('a4');

        return $pdf->download('manifestacao-'.$this->safeFilename($protocolo).'.pdf');
    }
    ...
    private function viewData(string $protocolo): array
    {
        ...
        return [
            ...
            'sections' => $this->accessProcesses->manifestacaoSections(),
            ...
            'manifestacaoLogs' => $this->manifestacaoLogs($protocolo),
        ];
    }

    private function logManifestacaoSave(Request $request, string $protocolo, array $campos): void
    {
        $user = $request->user();
    ...
    /**
     * @return array<int, array{user: string, date_created: string|null, campos: array<int, string>, area: string}>
     */
    private function manifestacaoLogs(string $protocolo): array
    {
        return DB::table('logs')
            ->where('log', 'like', '%"area":"manifestacao"%')
    ...
    {
        if ($count > 1) {
            return 'Manifestação bloqueada: este protocolo aparece em mais de um registro.';
        }

        return 'Manifestação bloqueada: protocolo não encontrado ou vazio.';
    }

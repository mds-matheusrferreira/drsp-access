...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm" aria-labelledby="importar-externo-title">
        ...
        <div class="space-y-6 p-6">
            <label for="externo-file" class="flex min-h-64 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-300 px-6 py-12 text-center transition-colors hover:border-blue-400 hover:bg-blue-50/40">
                ...
                <span id="externo-file-label" class="mt-2 text-sm font-medium text-gray-600">Arquivos Excel (.xlsx, .xls) - Máximo 10MB</span>
                <input id="externo-file" type="file" class="sr-only" accept=".xlsx,.xls">
            </label>

            <button id="externo-submit" type="button" disabled class="inline-flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-lg bg-gray-300 px-4 py-4 text-base font-semibold text-white transition-colors">
                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M7.5 7.5 12 3m0 0 4.5 4.5M12 3v13.5" />
                </svg>
    ...
    <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm" aria-labelledby="uploads-recentes-title">
        ...
        <div class="divide-y divide-gray-200">
            @foreach ([
                ['name' => 'dados_externos_2026_05.xlsx', 'date' => '29/05/2026 08:45', 'records' => '150 registros'],
                ['name' => 'dados_externos_2026_04.xlsx', 'date' => '28/04/2026 10:30', 'records' => '95 registros'],
                ['name' => 'dados_externos_2026_03.xlsx', 'date' => '25/03/2026 15:50', 'records' => '210 registros'],
            ] as $upload)
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const fileInput = document.getElementById('externo-file');
        const fileLabel = document.getElementById('externo-file-label');
        const submitButton = document.getElementById('externo-submit');

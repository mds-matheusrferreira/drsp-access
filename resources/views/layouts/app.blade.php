...
<body class="min-h-screen bg-gray-50 font-sans antialiased text-gray-800">
    @php
        ...
        $menuItems[] = ['label' => 'Plataformas', 'active' => false, 'icon' => 'layers', 'children' => [
            ['label' => 'Lecom', 'href' => 'https://cidadania.servicos.gov.br/workspace/', 'external' => true],
            ...
            <nav class="min-h-0 flex-1 overflow-y-auto py-4" aria-label="Menu principal">
                @foreach ($menuItems as $item)
                    @if (isset($item['children']))
                        <details class="group border-l-4 {{ $item['active'] ? 'border-white bg-white/10' : 'border-transparent' }}" @if ($item['active']) open @endif>
                            ...
                            <div class="mx-4 mb-3 max-h-[calc(100vh-12rem)] overflow-y-auto rounded-lg bg-white py-2 text-gray-700 shadow-xl">
                                @foreach ($item['children'] as $child)
                                    @if (($child['type'] ?? null) === 'heading')
                                        <p class="px-4 pb-1 pt-3 text-xs font-semibold uppercase tracking-wide text-gray-400 first:pt-1">{{ $child['label'] }}</p>
                                    @elseif (($child['type'] ?? null) === 'divider')
                                        <hr class="my-2 border-gray-200">
                                    @else
                                        <a href="{{ $child['href'] }}" class="block px-4 py-2 text-sm font-medium transition-colors {{ $child['active'] ?? false ? 'bg-blue-50 text-blue-700' : (($child['disabled'] ?? false) ? 'text-gray-400 hover:bg-gray-50' : 'hover:bg-blue-50 hover:text-blue-700') }}" @if ($child['disabled'] ?? false) aria-disabled="true" @endif @if ($child['external'] ?? false) target="_blank" rel="noopener noreferrer" @endif>
                                            {{ $child['label'] }}
                                        </a>
                                    @endif
                                @endforeach
                    ...
                    <div class="lg:hidden">
                        <details class="overflow-hidden rounded-lg bg-gradient-to-r from-blue-600 to-blue-800 text-white shadow-sm">
                            <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 [&::-webkit-details-marker]:hidden">
                                ...
                                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-white/10" aria-label="Abrir menu">
                                    ...
                                    @foreach ($menuItems as $item)
                                        @if (isset($item['children']))
                                            <details class="rounded-md" @if ($item['active']) open @endif>
                                                <summary class="flex cursor-pointer list-none items-center justify-between rounded-md px-3 py-2 text-sm font-semibold {{ $item['active'] ? 'bg-white/20' : 'hover:bg-white/10' }} [&::-webkit-details-marker]:hidden">
                                        ...
                                        @else
                                            <a href="{{ $item['href'] }}" class="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-semibold {{ $item['active'] ? 'bg-white/20' : 'hover:bg-white/10' }}" @if ($item['href'] === '#') aria-disabled="true" @endif @if ($item['external'] ?? false) target="_blank" rel="noopener noreferrer" @endif>
                                                @include('partials.menu-icon', ['name' => $item['icon']])
                                                {{ $item['label'] }}
                                            </a>
                                        @endif
